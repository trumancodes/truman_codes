...
export default function EmojiCanvas(props) {
  ...
  useEffect(() => {
    ...
    let leftWall = Bodies.rectangle(
      ...
      parent.offsetHeight + 26,
      50,
      parent.offsetHeight * 3,
      { isStatic: true },
      { fillStyle: "transparent" }
    );

    let rightWall = Bodies.rectangle(
      parent.offsetWidth + 26,
      parent.offsetHeight + 26,
      50,
      parent.offsetHeight * 3,
      { isStatic: true },
    ...
    window.addEventListener("resize", () => {
      ...
      timeout = setTimeout(() => {
        ...
        Body.setPosition(
          ground,
          Vector.create(parent.offsetWidth / 2, parent.offsetHeight + 26)
        );
        Body.setPosition(
          leftWall,
          Vector.create(-26, parent.offsetHeight + 26)
        );
        Body.setPosition(
          rightWall,
          Vector.create(parent.offsetWidth + 26, parent.offsetHeight + 26)
        );
      }, 250);
    });
    ...
    emojiSpots.forEach((emojiSpot) => {
      emojiSpot.addEventListener("mouseover", (e) => {
        ...
        if (e.clientX <= window.innerWidth / 2) {
          ...
          dropSpot = randomNumber(
            window.innerWidth - window.innerWidth / 4 + 50,
            window.innerWidth
          );
        }
